package com.liyaan.camera.camera1
...
class CameraActivity:AppCompatActivity() {
                ...
                if (!lock){
                    mCameraHelper.getCamera()?.let {
                        mMediaRecorderHelper =
                            MediaRecorderHelper(this@CameraActivity,
    ...
    private fun savePic(data: ByteArray?) {
        ...
        BitmapUtils.savePic(data, "camera1",
            mCameraHelper.mCameraFacing ==
                    Camera.CameraInfo.CAMERA_FACING_FRONT, { savedPath, time ->
            runOnUiThread {
                Log.i("aaaa","图片保存成功！ 保存路径：$savedPath 耗时：$time")
            }
        }, { msg ->
            runOnUiThread {
                Log.i("aaaa","图片保存失败！ $msg")
            }
        })
    }
