package com.liyaan.glide.util.pool
...
object FactoryPools {
    ...
    fun <T> threadSafeList(size: Int): Pools.Pool<MutableList<T>>{
        ...
        return build(Pools.SynchronizedPool<MutableList<T>>(size),factory, resetter)
    }
    ...
    fun <T> threadSafe(size:Int ,factory : Factory<T> ): Pools.Pool<T> {
        return build(Pools.SynchronizedPool<T>(size), factory)
    }
    ...
    private class FactoryPool<T>(val pool:Pools.Pool<T>,
                                 val factory:Factory<T>,
                                 val resetter: Resetter<T>):Pools.Pool<T>{
